...
function disable_microsoft_store_exe_stubs () {
  ...
  # On repeat runs, some stubs may have been recreated. To avoid name
  # conflicts with earlier backup copies of the same name, we can add
  # a date component to the backup path:
  BAK_DIR+="$(printf -- '%(%y%m%d-%H%M)T' -1)/"
